...
def get_big_mac_price_by_year(year,country_code):
    filtered = df[(df['date'].str.startswith(str(year))) & (df['iso_a3'].str.lower() == country_code.lower())]
...
if __name__ == "__main__":
    ...
    print(f"Cheapest Big Mac price in {year}: {get_the_most_expensive_big_mac_price_by_year(year)}")
    print(f"Most expensive Big Mac price in {year}: {get_the_most_expensive_big_mac_price_by_year(year)}")